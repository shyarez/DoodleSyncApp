...
export const StickyNote = ({ note, user, onMove, onResize, onChangeColor, onDelete, onTextChange }) => {
  const isOwner = !note.creatorId || note.creatorId === user?.userId;
  ...
  const onHeaderDown = (e) => {
    if (!isOwner || e.target.closest("[data-no-drag]")) return;
  ...
  // don't override while typing
  if (document.activeElement === textRef.current) return;

  if (textRef.current.innerText !== (note.text || "")) {
  ...
  return (
    <div style={{
      position: "absolute", left: note.x, top: note.y, width: note.w, height: note.h, zIndex: focused ? 25 : 20, userSelect: "none",
      ...
      <div style={{
        width: "100%", height: "100%", background: note.color, borderRadius: 6,
        ...
        <div
          ...
          onFocus={() => isOwner && setFocused(true)}
          ...
          style={{ flex: 1, padding: "8px 10px", outline: "none", fontSize: 14, color: "rgba(0,0,0,0.72)", fontFamily: "'Poppins',system-ui", lineHeight: 1.55, wordBreak: "break-word", overflowY: "auto", cursor: isOwner ? "text" : "default" }}
